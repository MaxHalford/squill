...
import type { OAuthConfigSource } from '../services/oauth/googleClientConfig'

const { confirm } = useDialog()

const callbackUrl = computed(() =>
  typeof window !== 'undefined'
    ? `${window.location.origin}/oauth/bigquery/callback`
    : '<your-origin>/oauth/bigquery/callback',
)
...
const recomputeSource = () => {
  if (oauthSettings.googleClientId && oauthSettings.googleClientSecret) {
    activeSource.value = 'user'
  } else {
    const bundledId = import.meta.env.VITE_GOOGLE_CLIENT_ID || ''
    const bundledSecret = import.meta.env.VITE_GOOGLE_CLIENT_SECRET || ''
      ...
      >
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p class="intro">
              By default Squill uses a bundled Google OAuth client to sign in to BigQuery. If your organization restricts third-party OAuth apps, paste a client ID and secret from your own GCP project here. Credentials are stored only in this browser.
            </p>
            ...
            >
              <ol class="help-list">
                ...
                <li>Add <code>{{ callbackUrl }}</code> as an authorized redirect URI.</li>
...
.source-user { color: var(--color-accent); font-weight: 600; }
.source-none { color: #dc3545; }
...
.input-label { font-size: var(--font-size-caption); color: var(--text-secondary); }
.text-input {
  width: 100%;
...
.secret-wrapper { display: flex; gap: var(--space-1); }
.secret-wrapper .text-input { flex: 1; }
.secret-toggle {
  padding: var(--space-1) var(--space-2);
...
.secret-toggle:hover { background: var(--surface-primary); }
.button-row {
  display: flex;
  gap: var(--space-2);
...
.help-list { margin: 0 0 var(--space-2) 0; padding-left: var(--space-3); }
.help-list li { margin-bottom: var(--space-1); }
.help-list code {
  font-family: var(--font-family-mono);
  background: var(--surface-primary);
  padding: 0 4px;
}
.help-list a { color: var(--color-accent); }
...
.modal-fade-leave-to .modal-content { transform: scale(0.95); }
</style>
