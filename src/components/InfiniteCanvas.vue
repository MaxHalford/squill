...
const fitToView = () => {
  ...
  const newZoom = Math.min(Math.max(Math.min(zoomX, zoomY, 1), 0.1), 5)
  ...
  zoom.value = newZoom
  pan.value = {
    x: rect.width / 2 - contentCenterX * newZoom,
    y: rect.height / 2 - contentCenterY * newZoom
  }
}
...
const isOverScrollableArea = (element: HTMLElement | null): boolean => {
  const scrollableClasses = ['table-container', 'sql-editor', 'query-editor', 'datasets-list', 'schema-browser', 'detail-content']
  let current = element
  while (current && current !== canvasRef.value) {
...
const handleMouseUp = () => {
  ...
  if (isPanning.value) {
    if (!isActuallyPanning.value) emit('canvas-click')
